...
array< array<double,nx>, 2> HLL_wave_speeds
  ...
  array<double,nx> csL = eos.soundspeed<nx>(primitiveLR[0][0],primitiveLR[0][2]);// see reconstruction, the definition is the same as conservativeLR
  array<double,nx> csR = eos.soundspeed<nx>(primitiveLR[1][0],primitiveLR[1][2]);// this value for Riemannn solver HLL is also reconstructed from the cell center

  // set the left and right wavespeeds according to HLL prescription
  for(int i=0; i<nx; i++){
    wavespeedLR[0][i] = primitiveLR[0][1][i] - csL[i];// IMPLEMENT ME
    wavespeedLR[1][i] = primitiveLR[1][1][i] + csR[i];// IMPLEMENT ME
  }
...
array<array<double,nx>,3> HLL_flux(const array<array<array<double,nx>,3>,2>& primitiveLR,
  ...
  fluxL = conservative_flux<nx>(primitiveLR[0],conservativeLR[0]);// construct the flux for conservative variables by using the value from LR interface
  fluxR = conservative_flux<nx>(primitiveLR[1],conservativeLR[1]);// inputs are primitiveLR, conservativeLR, eos, &max_wavespeed

  // get the left and right wave speeds
  // while we're at it, compute the maximum to pass back to determine timestep
  array<array<double,nx>,2> wavespeedLR = HLL_wave_speeds<nx>(primitiveLR,eos);// was implemented above
  *max_wavespeed = 0; //default
  ...
  for(int v=0; v<3; v++){
    for(int i=0; i<nx; i++){
      flux[v][i] = (wavespeedLR[1][i] * fluxL[v][i] - wavespeedLR[0][i] * fluxR[v][i] + wavespeedLR[0][i] * wavespeedLR[1][i] * (conservativeLR[1][v][i] - conservativeLR[0][v][i])) / (wavespeedLR[1][i] - wavespeedLR[0][i]);// IMPLEMENT ME
    }
  }
  // the other 2 situations
  for(int i=0; i<nx; i++){
